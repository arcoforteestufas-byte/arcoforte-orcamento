...
3. Dorsal/Contravento/Encaixe;Topo (por modulo);3017;Dorsal 2 6m;ceil((n_vaos * vao * n_modulos) / 6);tipo_arco != 'lanternim';
...
3. Dorsal/Contravento/Encaixe;Arco Interno;3027;Encaixe Central GALV;(n_vaos - 1) * n_modulos;tipo_arco != 'lanternim';
4. Lateral/Calha;Lateral 1 e 2 (superior);3035;Lateral Perfil Simples 6m;ceil((n_vaos * vao) / 6);cobertura == true and fechamento_lateral == false;
4. Lateral/Calha;Lateral 1 e 2 (superior);3034;Lateral Perfil Duplo 6m;ceil((n_vaos * vao) / 6);fechamento_lateral == true;
4. Lateral/Calha;Calha Lateral;5200;Calha Maxx 6m;ceil((n_vaos * vao) / 6);calha_lateral == true and vao == 3;
4. Lateral/Calha;Calha Lateral;5110;Calha Big Maxx 8m;ceil((n_vaos * vao) / 8);calha_lateral == true and vao == 4;
4. Lateral/Calha;Calha Central;5200;Calha Maxx 6m;ceil((n_vaos * vao * (n_modulos - 1)) / 6);n_modulos > 1 and vao == 3;
4. Lateral/Calha;Calha Central;5110;Calha Big Maxx 8m;ceil((n_vaos * vao * (n_modulos - 1)) / 8);n_modulos > 1 and vao == 4;
4. Lateral/Calha;Frente/Fundo (1 modulo);3042;Frontal 50x30;2;n_modulos == 1 and largura_modulo == 7;
4. Lateral/Calha;Frente/Fundo (1 modulo);3015;Frontal 50x30;2;n_modulos == 1 and largura_modulo == 8;
4. Lateral/Calha;Frente/Fundo (1 modulo);3016;Frontal 50x30;2;n_modulos == 1 and largura_modulo == 10;
4. Lateral/Calha;Frente/Fundo (2+ modulos);3035;Lateral Perfil;ceil((n_modulos * largura_modulo) / 6) * 2;n_modulos > 1 and fechamento_lateral == false;
4. Lateral/Calha;Frente/Fundo (2+ modulos);3034;Lateral Perfil Duplo;ceil((n_modulos * largura_modulo) / 6) * 2;n_modulos > 1 and fechamento_lateral == true;
4. Lateral/Calha;Fixacao Inferior (Mureta);5085;Perfil Aluminio Simples 6m;ceil(((n_vaos * vao) * 2 + (n_modulos * largura_modulo) * 2) / 6);fixacao_inferior == 'mureta';
5. Emendas/Suportes;Emenda Lateral;3063;Kit Emenda Par;max(0, ceil((n_vaos * vao) / 6) - 1) * (n_modulos == 1 ? 2 : 3);fechamento_lateral == true;
...
5. Emendas/Suportes;PU Selante;5024;PU Selante 400gr;ceil(((n_vaos * vao) / (vao * 2) * n_modulos) / 2.5);calha_lateral == true or n_modulos > 1;
6. Mola;Lateral Superior;3049;Mola de Fixacao 25m;ceil((n_vaos * vao * (cobertura == true and fechamento_lateral == true ? 4 : 2)) / 25);cobertura == true or fechamento_lateral == true;
...
9. Parafusos/Diversos;Geral;5107;Kit Parafuso 3x5/16;PENDENTE;PENDENTE;
9. Parafusos/Diversos;Geral;5114;Kit Parafuso 1x5/16;PENDENTE;PENDENTE;
9. Parafusos/Diversos;Frontal;3150;Anti-Camara Frontal;PENDENTE;PENDENTE;
9. Parafusos/Diversos;Lateral;3160;Anti-Camara Lateral;PENDENTE;PENDENTE;
10. Fechamento Lateral (Telas);Lateral/Frontal;TELA-XXX;Telas diversas;PENDENTE;PENDENTE;
...
csv.forEach(line => {
  ...
  if (!prodsSeen.has(prodCode) && prodCode !== 'TELA-XXX' && prodCode !== 'FILME-XXX') {
    prodInserts.push(`('${prodCode}', '${prodName}', 'un', 0)`);
    prodsSeen.add(prodCode);
  }
  ...
  const fSql = formula.replace(/'/g, "''");
  const cSql = cond.replace(/'/g, "''");
  ruleInserts.push(`('${posId}', '${prodCode}', '${fSql}', '${cSql}', 'un', 'nenhum')`);
});

const posSql = Object.entries(posIdMap).map(([key, uuid]) => {
  const gNum = key.split('-')[0];
  const pName = key.split('-').slice(1).join('-');
  return `INSERT INTO public.posicoes (id, grupo_id, nome) VALUES ('${uuid}', '${groups[gNum]}', '${pName}');`;
}).join('\n');
...
fs.writeFileSync('supabase/migrations/20260729170000_seed_remaining_rules.sql', sql);
